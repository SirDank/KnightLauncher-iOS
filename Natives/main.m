...
bool init_checkForJailbreak() {
  ...
  for (int i = 0; i < _dyld_image_count(); i++) {
    if (strcmp(_dyld_get_image_name(i), "/usr/lib/pspawn_payload-stg2.dylib") ==
        0) {
      return true;
    }
  }
...
void init_logDeviceAndVer(char *argument) {
  // Amethyst version
  NSLog(@"[Pre-Init] KnightLauncher INIT!");
  NSLog(@"[Pre-Init] Version: %@-%s",
        NSBundle.mainBundle.infoDictionary[@"CFBundleShortVersionString"],
        CONFIG_TYPE);
  NSLog(@"[Pre-Init] Commit: %s (%s)", CONFIG_COMMIT, CONFIG_BRANCH);

  NSString *tsPath = [NSString
      stringWithFormat:@"%@/../_TrollStore", NSBundle.mainBundle.bundlePath];
  const char *type;
...
void init_redirectStdio() {
  ...
  NSString *oldName =
      [home stringByAppendingPathComponent:@"latestlog.old.txt"];
...
void init_setupAccounts() {
  NSString *controlPath =
      [@(getenv("POJAV_HOME")) stringByAppendingPathComponent:@"accounts"];
  [fm createDirectoryAtPath:controlPath
...
void init_setupCustomControls() {
  NSString *controlPath =
      [@(getenv("POJAV_HOME")) stringByAppendingPathComponent:@"controlmap"];
  [fm createDirectoryAtPath:controlPath
      withIntermediateDirectories:NO
                       attributes:nil
                            error:nil];
  generateAndSaveDefaultControl();
  NSString *gamepadControlPath =
      [controlPath stringByAppendingPathComponent:@"gamepads"];
  [fm createDirectoryAtPath:gamepadControlPath
      withIntermediateDirectories:NO
                       attributes:nil
                            error:nil];
...
void init_setupMultiDir() {
  ...
  if (multidir.length == 0) {
    ...
    NSLog(@"[Pre-init] Game directory was not set. Defaulting to %@ for future "
          @"use.\n",
          multidir);
  } else {
  ...
  NSString *lasmPath = [NSString
      stringWithFormat:@"%s/Library/Application Support/minecraft", home];
  NSString *multidirPath =
  ...
  NSArray *dirsToCreate = @[
    ...
    lasmPath.stringByDeletingLastPathComponent, multidirPath
  ];
  ...
  [fm createSymbolicLinkAtPath:lasmPath
...
void init_setupResolvConf() {
  ...
  NSString *path =
      [NSString stringWithFormat:@"%s/resolv.conf", getenv("POJAV_HOME")];
  if (![fm fileExistsAtPath:path]) {
    [@"nameserver 8.8.8.8\n"
     @"nameserver 8.8.4.4" writeToFile:path
                            atomically:YES
...
void init_setupHomeDirectory() {
  setenv("HOME",
         [NSFileManager.defaultManager URLsForDirectory:NSDocumentDirectory
             ...
             .lastObject.path.stringByDeletingLastPathComponent.UTF8String,
         1);
  NSString *homeDir;
  NSError *homeError;

  BOOL isNotSandboxed =
      [@(getenv("HOME")).lastPathComponent isEqualToString:NSUserName()];
  homeDir =
      [NSString stringWithFormat:@"%s/Documents%@", getenv("HOME"),
                                 isNotSandboxed ? @"/KnightLauncher" : @""];
  ...
  if (homeError != nil) {
    ...
    homeError = nil;
    homeDir = NSSearchPathForDirectoriesInDomains(NSCachesDirectory,
...
int main(int argc, char *argv[]) {
  if (pJLI_Launch) {
    return pJLI_Launch(
        argc, (const char **)argv, 0,
        NULL,    // sizeof(const_jargs) / sizeof(char *), const_jargs,
        0, NULL, // sizeof(const_appclasspath) / sizeof(char *),
                 // const_appclasspath,
        "1.8.0-internal", "1.8",
  ...
  // If sandbox is disabled, W^X JIT can be enabled by KnightLauncher itself
  if (!isJITEnabled(true) &&
      getEntitlementValue(@"com.apple.private.security.no-sandbox")) {
    NSLog(@"[Pre-init] no-sandbox: YES, trying to enable JIT");
    int pid;
    int ret = posix_spawnp(&pid, argv[0], NULL, NULL,
                           (char *[]){argv[0], "", NULL}, environ);
    if (ret == 0) {
      ...
      NSLog(@"[Pre-init] Failed to enable JIT: posix_spawn() failed errno %d",
            errno);
    }
  }
