...
void internal_showDialog(NSString *title, NSString *message) {
  ...
  UIWindow *alertWindow =
      [[UIWindow alloc] initWithWindowScene:UIWindow.mainWindow.windowScene];
  alertWindow.frame = UIScreen.mainScreen.bounds;
  alertWindow.rootViewController = [UIViewController new];
  alertWindow.windowLevel = 1000;
  [alertWindow makeKeyAndVisible];
  objc_setAssociatedObject(alert, @selector(alertWindow), alertWindow,
...
JNIEXPORT void JNICALL Java_net_kdt_pojavlaunch_uikit_UIKit_showError(
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    UIAlertController *alert = [UIAlertController
        alertControllerWithTitle:title_o
                         message:message_o
                  preferredStyle:UIAlertControllerStyleAlert];
...
JNIEXPORT void JNICALL Java_net_kdt_pojavlaunch_uikit_UIKit_updateProgress(
    JNIEnv *env, jclass clazz, jstring status, jint progress, jint max) {
  NSString *status_o = nil;
  if (status != NULL) {
    const char *status_c = (*env)->GetStringUTFChars(env, status, 0);
    status_o = @(status_c);
    (*env)->ReleaseStringUTFChars(env, status, status_c);
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    NSMutableDictionary *userInfo = [NSMutableDictionary dictionary];
    if (status_o)
      userInfo[@"status"] = status_o;
    userInfo[@"progress"] = @(progress);
    userInfo[@"max"] = @(max);
    [[NSNotificationCenter defaultCenter]
        postNotificationName:@"KnightLauncherProgress"
                      object:nil
                    userInfo:userInfo];

    if (status_o) {
      NSLog(@"[KnightInstaller] Status: %@", status_o);
    }
  });
}
  ...
  if (action == CLIPBOARD_PASTE) {
    ...
    if (UIPasteboard.generalPasteboard.hasStrings) {
      return (*env)->NewStringUTF(
          env, [UIPasteboard.generalPasteboard.string UTF8String]);
    } else {
...
void UIKit_launchMinecraftSurfaceVC(UIWindow *window, NSDictionary *metadata) {
  // Leave this pref, might be useful later for launching with Quick
  // Actions/Shortcuts/URL Scheme
  // setPreference(@"internal_launch_on_boot",
  // getPreference(@"restart_before_launch"));
    ...
    [UIView animateWithDuration:0.2
        ...
        completion:^(BOOL b) {
          ...
          window.rootViewController =
              [[SurfaceViewController alloc] initWithMetadata:metadata];
...
void UIKit_returnToSplitView() {
  // Researching memory-safe ways to return from SurfaceViewController to the
  // split view so that the app doesn't close when quitting the game (similar
  // behaviour to Android)
  dispatch_async(dispatch_get_main_queue(), ^{
    UIWindow *window = UIWindow.mainWindow;

    // Return from JavaGUIViewController
    if ([window.rootViewController
            isKindOfClass:LauncherSplitViewController.class]) {
    ...
    [UIView animateWithDuration:0.2
        ...
        completion:^(BOOL b) {
          ...
          if (tmpRootVC) {
            ...
            window.rootViewController = [[LauncherSplitViewController alloc]
